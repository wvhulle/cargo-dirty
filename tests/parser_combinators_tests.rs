...
fn nom_parser_handles_env_var_changed_with_some_to_none() {
    ...
    assert_eq!(result, Some(RebuildReason::EnvVarChanged {
        name: "CC".to_string(),
...
fn nom_parser_handles_unit_dependency_info_changed() {
    ...
    assert_eq!(result, Some(RebuildReason::UnitDependencyInfoChanged {
...
fn nom_parser_handles_fs_status_outdated_with_file_change() {
    ...
    assert_eq!(result, Some(RebuildReason::FileChanged {
...
fn nom_parser_extracts_reason_from_complex_cargo_log_line() {
    ...
    if let Some(RebuildReason::EnvVarChanged { name, old_value, new_value }) = result {
        assert_eq!(name, "CC");
...
fn nom_parser_returns_none_for_lines_without_dirty_marker() {
    let log_line = r#"    0.102058909s  INFO cargo::core::compiler::fingerprint: some other log message"#;
    let result = parse_rebuild_reason(log_line);
